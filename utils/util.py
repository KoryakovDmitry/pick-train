...
def text_index_to_str(texts: torch.Tensor, mask: torch.Tensor):
    ...
    for i in range(B):
        decoded_text = []
        for idx, text_index in enumerate(union_texts[i]):
...
def texts_to_union_texts(texts_b, mask):
    ...
    union_boxes = torch.full_like(boxes, keys_vocab_cls['<pad>'], device=texts.device)

    max_seq_length = 0
    for i in range(B):
        valid_text = torch.masked_select(texts[i], mask[i].bool())
        valid_box = torch.masked_select(boxes[i], mask[i].bool())
        valid_length = valid_text.size(0)
        union_texts[i, :valid_length] = valid_text
        union_boxes[i, :valid_length] = valid_box
    ...
    union_boxes = union_boxes[:, :max_seq_length]

    # (B, N*T)
    return union_texts, union_boxes
