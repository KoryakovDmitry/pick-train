...
MAX_BOXES_NUM = 250  # limit max number boxes of every documents
...
class Document:
    def __init__(self, boxes_and_transcripts_file: Path, image_file: Path,
        ...
        try:
            ...
            # read image
            image_unresized = cv2.imread(image_file.as_posix())
        ...
        try:
            ...
            height, width, _ = image_unresized.shape

            # resize image
            image = cv2.resize(image_unresized, self.resized_image_size, interpolation=cv2.INTER_LINEAR)
            ...
            self.whole_image_unresized = image_unresized
